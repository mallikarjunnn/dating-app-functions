...
typedef struct {
    ...
    char lookingFor[MAX_INTEREST_LEN];  // New interest
    char gender;
    int age;
    bool isRegistered;
} UserProfile;
...
// Global array of user profiles, feedback stack, and user count
UserProfile users[MAX_USERS];
Feedback feedbackStack[MAX_USERS];
int userCount = 0;
int feedbackCount = 0;
...
void addFeedback(int loggedInUser) {
    if (loggedInUser == -1) {
        printf("Please log in to provide feedback.\n");
        return;
    }

    Feedback newFeedback;
    strcpy(newFeedback.userName, users[loggedInUser].name);

    printf("Enter your feedback (max %d characters): ", MAX_FEEDBACK_LEN);
    scanf(" %[^\n]", newFeedback.feedbackText);  // %[^\n] allows reading spaces

    if (feedbackCount < MAX_USERS) {
        feedbackStack[feedbackCount++] = newFeedback;
        printf("Feedback added successfully!\n");
    } else {
        printf("Feedback limit reached. Cannot add more feedback.\n");
    }
}
...
void displayFeedback() {
    ...
    for (int i = 0; i < feedbackCount; i++) {
        printf("%s: %s\n", feedbackStack[i].userName, feedbackStack[i].feedbackText);
    }
}
