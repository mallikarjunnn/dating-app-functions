...
void registerUser() {
  ...
  // Prompt for other user details
    ...
    // Initialize the rating to 0 for a new user
    newUser.rating = 0;
...
int loginUser() {
    ...
    for (int i = 0; i < userCount; i++) {
        if (strcmp(users[i].name, name) == 0 && strcmp(users[i].password, password) == 0) {
            ...
            printf("Enter a rating from 1 to 10 for user %s: ", users[i].name);
            scanf("%d", &users[i].rating);
            return i; // User index
        }
    }
...
void heapify(UserProfile arr[], int n, int i) {
    int largest = i;
    int left = 2 * i + 1;
    int right = 2 * i + 2;
...
// Heap sort function
void heapSort(UserProfile arr[], int n) {
...
// Display users sorted by rating
void displayUsersByRating() {
    heapSort(users, userCount);

    printf("\nUsers sorted by rating:\n");
    printf("%-20s %-10s\n", "Name", "Rating");
    for (int i = 0; i < userCount; i++) {
        printf("%-20s %-10d\n", users[i].name, users[i].rating);
    }
}
